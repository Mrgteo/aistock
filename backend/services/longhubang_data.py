"""
智瞰龙虎数据采集模块
使用StockAPI获取龙虎榜数据，失败时自动切换akshare备用数据源
"""
...
class LonghubangDataFetcher:
    ...
    def _check_akshare(self):
        ...
        try:
            ...
            print("[智瞰龙虎] akshare 未安装，将无法使用备用数据源")
            return False
    ...
    def _safe_request(self, url, params=None):
        ...
        Returns:
            dict: 响应数据；无数据时返回 {"code": 40400, "data": []} 以区分"真的没有数据"和"请求失败"
        """
        for attempt in range(self.max_retries):
            try:
                ...
                if response.status_code == 200:
                    ...
                    if code == 20000:
                        return data
                    elif code in self.API_CODE_NO_DATA:
                        # 明确返回"无数据"，不视为请求失败
                        print(f"    API提示: {data.get('msg', '无龙虎榜数据')}")
                        return {"code": 40400, "data": []}
                    else:
    ...
    def _fetch_from_akshare(self, date):
        """
        使用akshare获取指定日期的龙虎榜数据作为备用

        Args:
            date: 日期，格式为 YYYY-MM-DD

        Returns:
            dict: {"code": 20000, "data": [...]}
        """
        print(f"[智瞰龙虎] 通过akshare获取 {date} 的龙虎榜数据...")
        try:
            ...
            # 将DataFrame转换为与原始API兼容的字典列表格式
            # 原始字段：yzmc(游资名称), yyb(营业部), sblx(榜单类型),
            ...
            for _, row in df.iterrows():
                record = {
                    'yzmc': '龙虎榜',          # 泛指，无具体游资席位
                    'yyb': row.get('上榜原因', '龙虎榜'),
                    'sblx': row.get('上榜原因', '龙虎榜'),
                    'gpdm': str(row.get('代码', '')),
                    'gpmc': str(row.get('名称', '')),
                    'mrje': float(row.get('龙虎榜买入额', 0) or 0),
                    'mcje': float(row.get('龙虎榜卖出额', 0) or 0),
                    'jlrje': float(row.get('龙虎榜净买额', 0) or 0),
            ...
            return {"code": 20000, "data": records}
    ...
    def get_longhubang_data(self, date):
        ...
        if result and result.get('data'):
            ...
            return result
        elif result is not None and result.get('code') in self.API_CODE_NO_DATA:
            # 原始API无数据，尝试akshare备用
            if self._akshare_available:
                akshare_result = self._fetch_from_akshare(date)
                if akshare_result and akshare_result.get('data'):
